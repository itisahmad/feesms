...
import { parentSendOTP } from '@/lib/api';
...
export default function ParentRegisterPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await parentSendOTP(schoolCode.trim(), phone.trim());
  ...
  return (
    <ParentAuthShell subtitle="Create your account">
      <h2 className="mb-6 text-xl font-semibold text-gray-900">
        {step === 'details' ? 'Verify your phone' : 'Set password'}
      </h2>
            ...
            <form onSubmit={handleSendOTP} className="space-y-5">
              {error && (
                <div className="rounded-lg bg-red-50 px-3 py-2.5 text-sm text-red-700">
                  ...
                  {error.includes('Account already exists') ? (
                    <>
                      {' '}
                      <Link href="/parent/forgot-password" className="font-medium underline">
                        Forgot password?
                      </Link>
                    </>
                  ) : null}
              ...
              <div>
                ...
                <p className="mt-1.5 text-xs text-gray-500">
                  Must match the parent phone on your child&apos;s school record.
                </p>
              </div>
              <button
                type="submit"
